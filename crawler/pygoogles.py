...
class pygoogle:
        ...
        for page in range(0,self.pages):
            ...
            if print_results:
                ...
                if data.has_key('responseData') and data['responseData'].has_key('results'):
                    for result in  data['responseData']['results']:
                        if result:
                            compiler = "{}\n{}\n{}".format(urllib.unquote(result['titleNoFormatting']), result['content'].strip("<b>...</b>").replace("<b>",'').replace("</b>",'').replace("&#39;","'").strip(), urllib.unquote(result['unescapedUrl'])+'\n' )
                            responser.append(compiler)
                else:
                    self.logger.error('no responseData key found in response. very unusal')
            results.append(data)
            return responser
    def cont(self):
	    ...
	    for page in range(0,self.pages):
			    ...
			    if data.has_key('responseData') and data['responseData'].has_key('results'):
			                    for result in data['responseData']['results']:
			                        conteudo =  '[%s]\n %s \n%s'%(urllib.unquote(result['titleNoFormatting']),result['content'].strip("<b>...</b>").replace("<b>",'').replace("</b>",'').replace("&#39;","'").strip(), urllib.unquote(result['unescapedUrl'])+'\n')
